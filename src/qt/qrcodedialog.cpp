...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non tsc unit input in lnReqAmount, we generate the URI with tsc as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(testcoinUnits::format(testcoinUnits::tsc, ui->lnReqAmount->valueAsMpq()));
            paramCount++;
        }
        else
        {
